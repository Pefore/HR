...
{
    public class registerLYController : Controller
    {
        ...
        public ActionResult Index(int id)
        {
            ...
            ArrayList list = er.engage_resumeYouXianFenYe3(l);
        ...
        public ActionResult BTG()
        {
            ...
            if (er1.engage_resumeUpdate(erm) > 0)
            {
                ...
                if (er.engage_interviewUpdate(eim) > 0)
                {
                    return Content("<script>alert('释放简历成功');window.location.href='/registerLY/Index'</script>");
                }
                else
                {
                    return Content("<script>alert('释放简历失败');window.location.href='/registerLY/Update?id=" + eim.ein_id + "'</script>");
                }
            }
            else
            {
                return Content("<script>alert('释放简历失败');window.location.href='/registerLY/Update?id=" + ein_id + "'</script>");
            }
        }
    }
}